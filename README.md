...
- **model-fusion.md** — to produce an ARTIFACT: run FIRST on the architecture (Phase 0) via `.claude/bin/fusion-architect`: N models (2–3, distinct families) design independently and a blind merger reconciles them with consensus/divergence/`HUMAN DECISION NEEDED` provenance. The merger consolidates agreement but never adjudicates disagreement
- **second-opinion.md** — to make a DECISION: two families answer one question independently via the `second_opinion` tool and both answers come back unmerged, because a decision's value is in the divergence. Fires off a structural checklist (public interface / schema / auth / competing designs / low confidence), never off a judgement about whether it feels important
